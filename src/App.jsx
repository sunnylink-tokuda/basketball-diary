...
function newGame(){return {opponent:"",myScore:"",oppScore:"",playTime:"",shot2a:"",shot2m:"",shot3a:"",shot3m:"",ast:"",reb:"",gd:"",stl:"",tov:"",good:"",reflect:""};};
...
function sumGames(games){
  const s={shot2a:0,shot2m:0,shot3a:0,shot3m:0,ast:0,reb:0,gd:0,stl:0,tov:0,mins:0,count:0};
  games.forEach(g=>{
    ...
    s.gd+=parseInt(g.gd)||0; s.stl+=parseInt(g.stl)||0; s.tov+=parseInt(g.tov)||0;
    s.mins+=parseInt(g.playTime)||0; s.count++;
  });
  s.pts=s.shot2m*2+s.shot3m*3;
      ...
      {gameStats?(
        <div>
          ...
          <div style={{background:"#fff",borderRadius:"8px",padding:"10px",marginBottom:6}}>
            <div style={{display:"grid",gridTemplateColumns:"auto 1fr 1fr 1fr",gap:"5px 8px",alignItems:"center"}}>
              <span/><span style={{fontSize:11,color:"#888",textAlign:"center"}}>試投/決定</span><span style={{fontSize:11,color:"#888",textAlign:"center"}}>成功率</span><span/>
              <span style={{fontSize:12,fontWeight:500,color:"#333"}}>2P</span>
              <span style={{fontSize:13,textAlign:"center",color:"#333"}}>{gameStats.shot2m}/{gameStats.shot2a}</span>
              <span style={{fontSize:13,textAlign:"center",color:"#333"}}>{gameStats.p2Pct!=null?gameStats.p2Pct+"%":"-"}</span><span/>
              <span style={{fontSize:12,fontWeight:500,color:"#333"}}>3P</span>
              <span style={{fontSize:13,textAlign:"center",color:"#333"}}>{gameStats.shot3m}/{gameStats.shot3a}</span>
              <span style={{fontSize:13,textAlign:"center",color:"#333"}}>{gameStats.p3Pct!=null?gameStats.p3Pct+"%":"-"}</span><span/>
              <span style={{fontSize:12,fontWeight:500,color:"#333"}}>FG</span>
              <span style={{fontSize:13,textAlign:"center",color:"#333"}}>{gameStats.shot2m+gameStats.shot3m}/{gameStats.shot2a+gameStats.shot3a}</span>
              <span style={{fontSize:13,textAlign:"center",color:"#333"}}>{gameStats.fgPct!=null?gameStats.fgPct+"%":"-"}</span><span/>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6}}>
            {[["AST",gameStats.ast],["REB",gameStats.reb],["GD",gameStats.gd],["STL",gameStats.stl],["TO",gameStats.tov]].map(([l,v])=>(
              <div key={l} style={{background:"#fff",borderRadius:"8px",padding:"8px",textAlign:"center"}}>
                <p style={{fontSize:11,color:"#888",margin:"0 0 2px"}}>{l}</p>
...
function GameForm({game,onChange,onDelete,index,total}){
  const p2m=parseInt(game.shot2m)||0,p3m=parseInt(game.shot3m)||0;
  const calcPts=p2m*2+p3m*3;
  ...
  return(
    <div style={{border:`1px solid ${COLOR.game}44`,borderRadius:"12px",padding:"14px",marginBottom:14,background:"#fff"}}>
      ...
      <input placeholder="チーム名" value={game.opponent||""} onChange={e=>sf("opponent")(e.target.value)} style={{width:"100%",boxSizing:"border-box",marginBottom:12,padding:"7px 10px",borderRadius:"8px",border:"0.5px solid #ccc",fontSize:14,color:"#333",background:"#fff"}} />
      ...
      <input type="number" min="0" placeholder="例：20" value={game.playTime||""} onChange={e=>sf("playTime")(e.target.value)} style={{width:"100%",boxSizing:"border-box",marginBottom:12,padding:"7px 10px",borderRadius:"8px",border:"0.5px solid #ccc",fontSize:14,color:"#333",background:"#fff"}} />
      ...
      <div style={{background:"#f5f5f3",borderRadius:"8px",padding:"10px",marginBottom:12}}>
        <div style={{display:"grid",gridTemplateColumns:"40px 1fr 1fr",gap:"6px 10px",alignItems:"center",marginBottom:8}}>
          ...
          <span style={{fontSize:13,fontWeight:500,color:"#333"}}>2P</span>
          <input type="number" min="0" value={game.shot2a||""} onChange={e=>sf("shot2a")(e.target.value)} style={numInp} />
          <input type="number" min="0" value={game.shot2m||""} onChange={e=>sf("shot2m")(e.target.value)} style={numInp} />
          <span style={{fontSize:13,fontWeight:500,color:"#333"}}>3P</span>
          <input type="number" min="0" value={game.shot3a||""} onChange={e=>sf("shot3a")(e.target.value)} style={numInp} />
          <input type="number" min="0" value={game.shot3m||""} onChange={e=>sf("shot3m")(e.target.value)} style={numInp} />
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8,marginBottom:12}}>
        <StatBox label="アシスト" value={game.ast} onChange={sf("ast")} />
        <StatBox label="リバウンド" value={game.reb} onChange={sf("reb")} />
        <StatBox label="グッドDF" value={game.gd} onChange={sf("gd")} />
        <StatBox label="スティール" value={game.stl} onChange={sf("stl")} />
        <StatBox label="ターンオーバー" value={game.tov} onChange={sf("tov")} />
      </div>
      <label style={lbl}>この試合の上手くいったこと</label>
      <textarea rows={2} placeholder="この試合で良かったプレーなど" value={game.good||""} onChange={e=>sf("good")(e.target.value)} style={{width:"100%",boxSizing:"border-box",resize:"vertical",marginBottom:10,padding:"7px 10px",borderRadius:"8px",border:"0.5px solid #ccc",fontSize:14,color:"#333",background:"#fff"}} />
      <label style={lbl}>この試合の反省点</label>
      <textarea rows={2} placeholder="この試合で改善したいこと" value={game.reflect||""} onChange={e=>sf("reflect")(e.target.value)} style={{width:"100%",boxSizing:"border-box",resize:"vertical",padding:"7px 10px",borderRadius:"8px",border:"0.5px solid #ccc",fontSize:14,color:"#333",background:"#fff"}} />
    </div>
  );
}
  ...
  return(
    <div style={{marginBottom:10}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:4,marginBottom:4}}>
        {[["2P",(game.shot2m||0)+"/"+(game.shot2a||0)],["3P",(game.shot3m||0)+"/"+(game.shot3a||0)],["得点",calcPts+"点"],["AST",game.ast||"-"],["REB",game.reb||"-"]].map(([l,v])=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:4,marginBottom:6}}>
        {[["GD",game.gd],["STL",game.stl],["TO",game.tov]].map(([l,v])=>(
          <div key={l} style={{background:"#fff",borderRadius:"8px",padding:"5px 4px",textAlign:"center"}}>
            <p style={{fontSize:10,color:"#888",margin:"0 0 2px"}}>{l}</p>
            <p style={{fontSize:13,fontWeight:500,color:"#333",margin:0}}>{v||"-"}</p>
          </div>
...
export default function App(){
  ...
  function openDay(day){setSel(fmtDate(calYear,calMonth,day));setEditMode(null);setView("day");}
  ...
  async function delRecord(type){
    ...
    if(Object.keys(rec).length===0){
      ...
      setRecords(prev=>{const n={...prev};delete n[sel];return n;});
    } else {
      await saveToSupabase(sel,rec);
      setRecords(prev=>({...prev,[sel]:rec}));
    }
    setSaving(false);
    setEditMode(null);
  }
  ...
  if(loading) return(
    <div style={{padding:"3rem",textAlign:"center",color:"#888",fontSize:15}}>読み込み中...</div>
  );
  ...
  if(view==="day"){
    ...
    if(editMode==="solo") return(
      <div style={{padding:"1rem",maxWidth:520,margin:"0 auto"}}>
        ...
        <textarea rows={4} placeholder="気づきや課題など" value={soloForm.memo||""} onChange={e=>setSoloForm(f=>({...f,memo:e.target.value}))} style={{width:"100%",boxSizing:"border-box",resize:"vertical",marginBottom:16,padding:"7px 10px",borderRadius:"8px",border:"0.5px solid #ccc",fontSize:14,color:"#333",background:"#fff"}} />
    ...
    if(editMode==="team"){
      const fields=[{k:"content",l:"練習内容",ph:"今日の練習メニュー"},{k:"taught",l:"教えてもらったこと",ph:"コーチや先輩から"},{k:"good",l:"上手くできたこと",ph:"うまくいったプレー"},{k:"improve",l:"改善すること",ph:"次につながる課題"},{k:"next",l:"次回の練習に向けて",ph:"次回意識したいこと"}];
      return(
        <div style={{padding:"1rem",maxWidth:520,margin:"0 auto"}}>
          <button onClick={()=>setEditMode(null)} style={btnS()}>← 戻る</button>
          ...
          {fields.map(({k,l,ph})=>(
            ...
            <textarea rows={3} placeholder={ph} value={teamForm[k]||""} onChange={e=>setTeamForm(f=>({...f,[k]:e.target.value}))} style={{width:"100%",boxSizing:"border-box",resize:"vertical",marginBottom:12,padding:"7px 10px",borderRadius:"8px",border:"0.5px solid #ccc",fontSize:14,color:"#333",background:"#fff"}} /></div>
          ))}
      ...
      <div style={{padding:"1rem",maxWidth:520,margin:"0 auto"}}>
        ...
        <div style={{borderTop:`2px solid ${COLOR.game}44`,paddingTop:16,marginBottom:16}}>
          ...
          {[{k:"memo",l:"今日の試合の感想",ph:"全体を通して感じたこと"},{k:"good",l:"今日の試合の上手くいったこと",ph:"今日良かったプレーや成長を感じたこと"},{k:"reflect",l:"今日の試合の反省点",ph:"今日改善すべきだったこと"},{k:"next",l:"次回に向けて",ph:"次の試合で意識したいこと"}].map(({k,l,ph})=>(
            <div key={k}><label style={lbl}>{l}</label><textarea rows={2} placeholder={ph} value={daySummary[k]||""} onChange={e=>setDaySummary(f=>({...f,[k]:e.target.value}))} style={{width:"100%",boxSizing:"border-box",resize:"vertical",marginBottom:12,padding:"7px 10px",borderRadius:"8px",border:"0.5px solid #ccc",fontSize:14,color:"#333",background:"#fff"}} /></div>
          ))}
        </div>
        <div style={{display:"flex",gap:8}}>
            ...
            {totals&&games.length>1&&(
              <div style={{background:"#fff",borderRadius:"8px",padding:"10px 12px",marginBottom:14,border:`0.5px solid ${COLOR.game}44`}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:4,borderTop:"0.5px solid #ddd",paddingTop:6,marginTop:4}}>
                  {[["AST",totals.ast],["REB",totals.reb],["GD",totals.gd],["STL",totals.stl],["TO",totals.tov]].map(([l,v])=>(
      ...
      <div style={{borderTop:"0.5px solid #ddd",paddingTop:16}}>
        <div style={{display:"flex",gap:6,marginBottom:14}}>
          {[{id:"month",label:`${calMonth+1}月の集計`},{id:"year",label:`${calYear}年の集計`}].map(({id,label})=>(
            <button key={id} onClick={()=>setStatsTab(id)} style={{...btnS(),flex:1,fontWeight:statsTab===id?500:400,borderColor:statsTab===id?"#888":"#ccc",background:statsTab===id?"#f5f5f3":"#fff"}}>
              {label}
            </button>
          ))}
        </div>
